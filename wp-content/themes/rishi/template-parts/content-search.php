...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'rt-supports-deeplink' ); echo rishi__cb_customizer_schema_org_definitions( 'article' ); ?><?php echo rishi_frontend_deeplink_customizer_preview( 'border-dashed','search' ); ?>>
					...
					if( $search_page_layout == 'listing' ){
						foreach( $search_structure as $structure ){
							...
							if( $structure['enabled'] == true && $structure['id'] == 'featured_image' ){
								echo rishi_single_featured_image( 'search', $structure['featured_image_ratio'],$img_scale, $structure['featured_image_size'], $structure['featured_image_visibility'] );
							}
						} ?>
