...
if (!function_exists('rishi__cb_customizer_get_patterns_svgs_list')) {
	function rishi__cb_customizer_get_patterns_svgs_list(  )
	{
		return array(
...
if (!function_exists('rishi__cb_customizer_get_svg_pattern')) {
	function rishi__cb_customizer_get_svg_pattern( $name, $color = '#ccc' )
	{
		$opacity = 1;
		...
		$patterns = rishi__cb_customizer_get_patterns_svgs_list();
...
if (!function_exists('rishi__cb_customizer_output_background_css')) {
	function rishi__cb_customizer_output_background_css( $args = [] ) {
		$args = wp_parse_args(
			$args,
			[
				'value' => [],
		...
		if (!$args['responsive']) {
			rishi__cb_customizer_output_single_background_css([
					'value' => $args['value'],
					'css' => $args['css'],
	...
	rishi__cb_customizer_assert_args($args, ['tablet_css', 'mobile_css']);
	...
	rishi__cb_customizer_output_single_background_css([
		...
		if (!rishi__cb_customizer_values_are_similar(
...
if (!function_exists('rishi__cb_customizer_output_single_background_css')) {
	function rishi__cb_customizer_output_single_background_css( $args = [] )
	{
		$args = wp_parse_args(
			$args,
			[
		...
		$args['value'] = rishi__cb_customizer_background_default_value($args['value']);

		$backgroundColor = rishi__cb_customizer_get_colors(
		 rishi__cb_get_akv(
				'backgroundColor',
				$args['value'],
		...
		$overlayColor = rishi__cb_customizer_get_colors(
		 rishi__cb_get_akv(
				'overlayColor',
				$args['value'],
				['default' => ['color' => '#e5e7ea']]
		...
		if ($args['value']['background_type'] === 'image') {
			$background_image = rishi__cb_get_akv('background_image', $args['value'], [
				'attachment_id' => null,
				'x' => 0.5,
				'y' => 0.5
			]);
			...
			if (!empty($attachment_url)) {
				...
				$background_position = rishi__cb_customizer_maybe_append_important(
				...
				$background_size = rishi__cb_customizer_maybe_append_important(
				 rishi__cb_get_akv('background_size', $args['value'], 'auto'),
					$args['important']
				);
				...
				$background_attachment = rishi__cb_customizer_maybe_append_important( rishi__cb_get_akv(
				...
				$background_repeat = rishi__cb_customizer_maybe_append_important( rishi__cb_get_akv(
					'background_repeat',
					$args['value'],
					'no-repeat'
				), $args['important']);
		...
		if ($args['value']['background_type'] === 'pattern') {
			$patternColor = rishi__cb_customizer_get_colors(
			 rishi__cb_get_akv(
					'patternColor',
					$args['value'],
					['default' => ['color' => '#e5e7ea']]
			...
			$image_url = 'url("' . rishi__cb_customizer_get_svg_pattern(
			 rishi__cb_get_akv('background_pattern', $args['value'], 'type-1'),
				$patternColor['default']
			) . '")';
		}

		if ($args['value']['background_type'] === 'gradient') {
			$gradient = rishi__cb_get_akv('gradient', $args['value'], '');
		...
		$image_url = rishi__cb_customizer_maybe_append_important($image_url, $args['important']);
		...
		if ($image_url !== 'none' && $image_url !== 'CT_CSS_SKIP_RULE') {
			$overlayColor['default'] = rishi__cb_customizer_maybe_append_important(
		...
		$backgroundColor['default'] = rishi__cb_customizer_maybe_append_important(
...
if (!function_exists('rishi__cb_customizer_background_default_value')) {
	function rishi__cb_customizer_background_default_value( $values = [] )
	{
